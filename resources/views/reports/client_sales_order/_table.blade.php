    ...
    <div class="col-sm-12">
        ...
        <table class="table table-bordered dataTable table-hover table-checkable
            ...
            <tr>
                ...
                <th class="sorting
                    {{(isset(request()->sortby) && request()->sortby=='final_total') ? 'sorting_'.request()->sorttype :''}}"
                    data-sorting_type="{{(isset(request()->sorttype) && request()->sortby=='final_total') ? request()->sorttype :'asc'}}"
                    data-column_name="final_total" style="cursor: pointer">
                    {!! trans('main.final_total') !!}<span id="final_total_icon"></span></th>
                <th class="sorting
                    {{(isset(request()->sorttype) && request()->sortby=='discount') ? 'sorting_'.request()->sorttype :''}}"
                    data-sorting_type="{{(isset(request()->sorttype) && request()->sortby=='discount') ? request()->sorttype :'asc'}}"
                    data-column_name="discount" style="cursor: pointer">
                    {{__('main.discount')}}
